...
import { generateAttendanceQRLink, parseAttendanceSessionFromUrl } from "./environment";
import {
  ...
  Bot, Send, Sparkles, FileText, DoorOpen, Award, Home, UtensilsCrossed,
  Megaphone, Wrench, ArrowRight, Zap, Layers, MessageSquare, Eye, Hash
} from "lucide-react";
...
function classifyComplaint(text){
  ...
  const cats=[["Electrical",/fan|light|switch|socket|electric|power|wiring/],["Plumbing",/water|tap|leak|pipe|plumb|drain|toilet|bathroom/],["Cleaning",/clean|dirty|dust|garbage|trash|sweep|mop|hygiene/],["Furniture",/chair|table|bed|desk|cupboard|door|window|lock/],["Internet",/wifi|internet|network|lan|router/],["Pest Control",/cockroach|rat|ant|pest|insect|bug/]];
  const locs=[["Hostel Block A",/block a|hostel a/],["Hostel Block B",/block b|hostel b/],["Hostel Block C",/block c|hostel c/],["Main Hostel",/hostel|room/],["Mess",/mess|canteen|kitchen/],["Academic Block",/class|lab|academic/]];
  const category=(cats.find(([,r])=>r.test(t))||["General"])[0];
  const location=(locs.find(([,r])=>r.test(t))||["Campus"])[0];
  const priority=/(urgent|emergency|danger|critical|not working|broken|flood|spark)/.test(t)?"High":/(leak|issue|problem|damage)/.test(t)?"Medium":"Low";
  const departments={Electrical:"Electrical Dept",Plumbing:"Plumbing Dept",Cleaning:"Housekeeping",Furniture:"Maintenance",Internet:"IT Dept","Pest Control":"Housekeeping",General:"Maintenance"};
  return {category,location,priority,department:departments[category]||"Maintenance"};
}
function detectRecurringIssues(complaints){
  const groups={};
  complaints.forEach(c=>{
    const key=`${c.aiCategory||"General"}-${c.aiLocation||"Campus"}`;
    if(!groups[key])groups[key]={key,category:c.aiCategory,location:c.aiLocation,count:0,items:[]};
    groups[key].count++;groups[key].items.push(c);
  });
  return Object.values(groups).filter(g=>g.count>=2).sort((a,b)=>b.count-a.count);
}

const demoNotices=[
  {id:"NOT-001",title:"Mid-Semester Exam Schedule Published",body:"The mid-semester examination schedule for all branches has been published. Students are advised to check the examination portal.",category:"Academic",target:"All",author:"Exam Controller",date:"2026-09-10",read:false},
  {id:"NOT-002",title:"Hostel Mess Menu Updated",body:"The weekly mess menu has been updated effective September 15. Check the Mess Menu section for details.",category:"Hostel",target:"Hostel Students",author:"Hostel Warden",date:"2026-09-09",read:false},
  {id:"NOT-003",title:"Annual Sports Day Registration Open",body:"Register for Annual Sports Day events before September 20. Visit the Sports Complex for forms.",category:"Events",target:"All",author:"Sports Committee",date:"2026-09-08",read:true},
  {id:"NOT-004",title:"Library Hours Extended",body:"The central library will remain open until 11 PM during exam weeks starting September 15.",category:"General",target:"All",author:"Librarian",date:"2026-09-07",read:true}
];
const demoGatePasses=[
  {id:"GP-10001",studentName:"Demo Student",studentId:"STU-DEMO-1",date:"2026-09-12",time:"02:00 PM",returnDate:"2026-09-12",returnTime:"06:00 PM",reason:"Medical appointment at city hospital",status:"Approved",approvedBy:"Warden Mr. Singh",gateVerified:false,createdAt:"2026-09-11"},
  {id:"GP-10002",studentName:"Demo Student",studentId:"STU-DEMO-1",date:"2026-09-15",time:"09:00 AM",returnDate:"2026-09-15",returnTime:"01:00 PM",reason:"Family visit",status:"Pending",approvedBy:"",gateVerified:false,createdAt:"2026-09-13"}
];
const demoCertificateRequests=[
  {id:"CERT-5001",studentName:"Demo Student",studentId:"STU-DEMO-1",type:"Bonafide Certificate",purpose:"Bank loan application",status:"Generated",requestDate:"2026-09-05",completedDate:"2026-09-07",verifiedBy:"Admin Office"},
  {id:"CERT-5002",studentName:"Demo Student",studentId:"STU-DEMO-1",type:"Character Certificate",purpose:"Internship application",status:"Submitted",requestDate:"2026-09-12",completedDate:"",verifiedBy:""}
];
const demoHostelComplaints=[
  {id:"HC-2001",studentName:"Demo Student",studentId:"STU-DEMO-1",description:"Hostel room B-204 ceiling fan not working since yesterday",aiCategory:"Electrical",aiLocation:"Hostel Block B",aiPriority:"High",aiDepartment:"Electrical Dept",assignedTo:"Rajesh Kumar",status:"In Progress",remarks:"Electrician assigned, spare fan ordered",createdAt:"2026-09-10"},
  {id:"HC-2002",studentName:"Neha Kapoor",studentId:"STU-DEMO-2",description:"Water leaking from bathroom tap in hostel A room 105",aiCategory:"Plumbing",aiLocation:"Hostel Block A",aiPriority:"Medium",aiDepartment:"Plumbing Dept",assignedTo:"Suresh Pal",status:"Resolved",remarks:"Tap washer replaced",createdAt:"2026-09-08"},
  {id:"HC-2003",studentName:"Aman Deep",studentId:"STU-DEMO-3",description:"Hostel Block B corridor lights are not working on 2nd floor",aiCategory:"Electrical",aiLocation:"Hostel Block B",aiPriority:"Medium",aiDepartment:"Electrical Dept",assignedTo:"",status:"Open",remarks:"",createdAt:"2026-09-12"}
];
const demoMessMenu={
  Monday:{breakfast:"Poha, Bread Butter, Tea/Coffee",lunch:"Rice, Dal Tadka, Aloo Gobi, Roti, Salad",snacks:"Samosa, Tea",dinner:"Rice, Paneer Butter Masala, Roti, Raita"},
  Tuesday:{breakfast:"Idli Sambhar, Banana, Tea/Coffee",lunch:"Rice, Rajma, Bhindi Fry, Roti, Pickle",snacks:"Bread Pakora, Coffee",dinner:"Rice, Chole, Roti, Mixed Veg, Kheer"},
  Wednesday:{breakfast:"Paratha, Curd, Tea/Coffee",lunch:"Rice, Dal Fry, Cabbage Sabzi, Roti, Salad",snacks:"Vada Pav, Tea",dinner:"Rice, Egg Curry / Paneer, Roti, Papad"},
  Thursday:{breakfast:"Upma, Bread Jam, Tea/Coffee",lunch:"Biryani / Veg Pulao, Raita, Roti, Salad",snacks:"Maggi, Juice",dinner:"Rice, Malai Kofta, Roti, Pickle, Gulab Jamun"},
  Friday:{breakfast:"Chole Bhature, Tea/Coffee",lunch:"Rice, Moong Dal, Baingan Bharta, Roti, Salad",snacks:"Pav Bhaji, Lassi",dinner:"Rice, Mix Veg, Roti, Dal Makhani, Ice Cream"},
  Saturday:{breakfast:"Dosa, Chutney, Tea/Coffee",lunch:"Rice, Kadhi Pakora, Aloo Jeera, Roti, Salad",snacks:"Dhokla, Tea",dinner:"Rice, Chicken Curry / Shahi Paneer, Roti, Sweet"},
  Sunday:{breakfast:"Poori Bhaji, Fruit, Tea/Coffee",lunch:"Special Thali: Rice, Dal, 2 Sabzi, Roti, Sweet, Salad",snacks:"Pasta, Cold Drink",dinner:"Rice, Butter Chicken / Paneer Tikka, Naan, Raita"}
};
...
function App(){
  ...
  const [gatePasses,setGatePasses]=useState(load("cf_gate_passes",demoGatePasses));
  const [certificateRequests,setCertificateRequests]=useState(load("cf_certificates",demoCertificateRequests));
  const [hostelComplaints,setHostelComplaints]=useState(load("cf_hostel_complaints",demoHostelComplaints));
  const [notices,setNotices]=useState(load("cf_notices",demoNotices));

  // Detect /student-attendance route from QR code scan
  useEffect(()=>{
    const path = window.location.pathname;
    if (path === "/student-attendance" || path.endsWith("/student-attendance")) {
  ...
  useEffect(()=>save("cf_certificates",certificateRequests),[certificateRequests]);
  useEffect(()=>save("cf_hostel_complaints",hostelComplaints),[hostelComplaints]);
  ...
  const studentNav=[
    ...
    ["history","Attendance History",History],["timetable","Daily Timetable",CalendarDays],
    ["notices","Notices & Announcements",Megaphone],["gatepass","Leave / Gate Pass",DoorOpen],
    ["certificate","Certificate Request",Award],["hostelcomplaint","Hostel Complaint",Home],
    ["messmenu","Mess Menu",UtensilsCrossed],["payments","Fees / Dues",Receipt],
    ["map","Campus Map",Map],["feedback","Feedback",ClipboardList],["settings","Settings",Settings]
  ];
  const adminNav=[
    ["dashboard","Dashboard",LayoutDashboard],["create","Create Session",QrCode],
    ["monitor","Live Monitoring",Activity],["students","Student Records",Users],["marks","Student Marks",ClipboardList],
    ["adminnotices","Notice Management",Megaphone],["admingatepass","Gate Pass Management",DoorOpen],
    ["admincert","Certificate Management",Award],["adminhostel","Complaint Management",AlertTriangle],
    ["map","Campus Map",Map],["manage","Manage Campus",Building2],["salary","Teacher Salaries",CircleDollarSign],
    ["feedback","Feedback",ClipboardList],["settings","Settings",Settings]
  ];
  const staffNav=[
    ["dashboard","Dashboard",LayoutDashboard],["staffcomplaints","Assigned Complaints",Wrench],
    ["staffupdate","Update Status",CheckCircle2],["settings","Settings",Settings]
  ];
  const nav=role==="student"?studentNav:role==="staff"?staffNav:adminNav;
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobile?"open":""}`}>
      <div className="brand"><div className="brand-orb"><Layers size={22}/></div><div><b>CAMPUS</b><span>FLOW</span></div><button className="icon-btn mobile-close" onClick={()=>setMobile(false)}><X/></button></div>
      ...
      <div className="sidebar-bottom">
        <button className="nav ai-nav" onClick={()=>setAssistantOpen(true)}><Bot size={19}/><span>CampusFlow AI</span></button>
    ...
    <main className="main">
      <header className="topbar">
        ...
        <div className="crumb"><span>CampusFlow</span><ChevronRight size={14}/><b>{nav.find(x=>x[0]===page)?.[1] || "Notifications"}</b></div>
        <div className="top-actions">
          ...
          <button className="notification-btn ai-top-btn" onClick={()=>setAssistantOpen(true)} aria-label="Open CampusFlow AI"><Bot size={19}/></button>
          <button className="icon-btn" title="Settings" onClick={()=>setPage("settings")}><Settings size={18}/></button>
          <div className="user-chip" onClick={()=>setPage("settings")} style={{cursor:"pointer"}} title="View Profile & Settings">
            <div className="avatar">{initials(profile.name)}</div>
            <div><b>{profile.name}</b><small>{role==="student"?"Student":role==="staff"?"Staff":"Admin"}</small></div>
          </div>
          <button className="icon-btn logout-top-btn" title="Logout" onClick={logout} aria-label="Logout"><LogOut size={18}/></button>
        </div>
      </header>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={page} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:.2}}>
            {page==="dashboard" && (role==="student"
              ? <StudentDashboard go={setPage} attendance={attendance} payments={payments} studentName={profile.name} studentId={profile.studentId} branch={profile.branch||profile.department} gatePasses={gatePasses} certificateRequests={certificateRequests} hostelComplaints={hostelComplaints} notices={notices}/>
              : role==="staff" ? <StaffDashboard go={setPage} complaints={hostelComplaints} profile={profile} setComplaints={setHostelComplaints}/>
              : <AdminDashboard go={setPage} attendance={attendance} session={session} students={registeredStudents} gatePasses={gatePasses} certificateRequests={certificateRequests} hostelComplaints={hostelComplaints} notices={notices}/>) }
            ...
            {page==="settings" && <SettingsPage theme={theme} setTheme={setTheme} logout={logout} profile={profile} role={role}/>}
            {page==="notices" && <NoticesPage notices={notices}/>}
            {page==="gatepass" && <GatePassPage profile={profile} gatePasses={gatePasses} setGatePasses={setGatePasses} setNotifications={setNotifications}/>}
            {page==="certificate" && <CertificateRequestPage profile={profile} certificateRequests={certificateRequests} setCertificateRequests={setCertificateRequests}/>}
            {page==="hostelcomplaint" && <HostelComplaintPage profile={profile} complaints={hostelComplaints} setComplaints={setHostelComplaints} setNotifications={setNotifications}/>}
            {page==="messmenu" && <MessMenuPage/>}
            {page==="adminnotices" && <AdminNoticesPage notices={notices} setNotices={setNotices}/>}
            {page==="admingatepass" && <AdminGatePassPage gatePasses={gatePasses} setGatePasses={setGatePasses} setNotifications={setNotifications}/>}
            {page==="admincert" && <AdminCertificatePage certificateRequests={certificateRequests} setCertificateRequests={setCertificateRequests}/>}
            {page==="adminhostel" && <AdminHostelComplaintPage complaints={hostelComplaints} setComplaints={setHostelComplaints}/>}
            {page==="staffcomplaints" && <StaffComplaintsPage complaints={hostelComplaints} setComplaints={setHostelComplaints} profile={profile}/>}
            {page==="staffupdate" && <StaffComplaintsPage complaints={hostelComplaints} setComplaints={setHostelComplaints} profile={profile}/>}
          </motion.div>
        </AnimatePresence>
      </div>
    </main>
...
function StudentDashboard({go,attendance,payments,studentName,studentId,branch,gatePasses=[],certificateRequests=[],hostelComplaints=[],notices=[]}){
  ...
  const activeGatePass=gatePasses.find(g=>g.studentId===studentId||g.studentId==="STU-DEMO-1");
  const activeCertificate=certificateRequests.find(c=>c.studentId===studentId||c.studentId==="STU-DEMO-1");
  const myComplaints=hostelComplaints.filter(c=>c.studentId===studentId||c.studentId==="STU-DEMO-1");
  const latestNotice=notices[0];

  return <>
    <PageTitle eyebrow="CAMPUSFLOW · STUDENT PORTAL" title={`Welcome back, ${studentName}.`} desc="Your unified gateway for academics, hostel services, requests, and campus life." actions={<button className="primary" onClick={()=>go("attendance")}><QrCode/> Scan Attendance</button>}/>
    <div className="stats-grid">
      <Stat icon={Activity} label="Overall Attendance" value={`${overall}%`} sub="Semester average"/>
      <Stat icon={DoorOpen} label="Gate Pass Status" value={activeGatePass?.status||"None"} sub={activeGatePass?`Valid ${activeGatePass.date}`:"Apply for leave"}/>
      <Stat icon={Award} label="Certificates" value={activeCertificate?.status||"None"} sub={activeCertificate?.type||"Ready to request"}/>
      <Stat icon={AlertTriangle} label="My Complaints" value={myComplaints.filter(c=>c.status!=="Closed").length} sub="Active hostel tickets"/>
    </div>
    ...
    <div className="cf-service-shortcuts">
      ...
      <div className="cf-shortcuts-grid">
        <button className="cf-shortcut-card card-orange" onClick={()=>go("gatepass")}>
          <div className="cf-icon-wrap"><DoorOpen size={22}/></div>
          <div>
            <b>Leave / Gate Pass</b>
            <span>{activeGatePass ? `${activeGatePass.status}: ${activeGatePass.reason.slice(0,25)}...` : "Apply for leave or exit permit"}</span>
          </div>
          <ChevronRight size={18}/>
        </button>
        <button className="cf-shortcut-card card-blue" onClick={()=>go("certificate")}>
          <div className="cf-icon-wrap"><Award size={22}/></div>
          <div>
            <b>Certificate Request</b>
            <span>{activeCertificate ? `${activeCertificate.status}: ${activeCertificate.type}` : "Bonafide, TC, Character certs"}</span>
          </div>
          <ChevronRight size={18}/>
        </button>
        <button className="cf-shortcut-card card-red" onClick={()=>go("hostelcomplaint")}>
          <div className="cf-icon-wrap"><Home size={22}/></div>
          <div>
        ...
        <button className="cf-shortcut-card card-purple" onClick={()=>go("notices")}>
          <div className="cf-icon-wrap"><Megaphone size={22}/></div>
          <div>
            <b>Notices & Announcements</b>
            <span>{latestNotice ? latestNotice.title.slice(0,26)+"..." : "Targeted broadcast feed"}</span>
          </div>
          <ChevronRight size={18}/>
        </button>
        <button className="cf-shortcut-card card-green" onClick={()=>go("messmenu")}>
          <div className="cf-icon-wrap"><UtensilsCrossed size={22}/></div>
          <div>
        ...
        <button className="cf-shortcut-card card-cyan" onClick={()=>go("payments")}>
          <div className="cf-icon-wrap"><Receipt size={22}/></div>
          <div>
            <b>Fees & Dues</b>
            <span>{payment ? `Due: INR ${Number(payment.due||0).toLocaleString("en-IN")}` : "Fee records & payment slips"}</span>
          </div>
          <ChevronRight size={18}/>
        </button>
      </div>
    ...
    <div className="dashboard-grid">
      <section className="panel"><div className="panel-head"><div><h3>Today’s Schedule</h3><p>Academic classes</p></div><button className="ghost" onClick={()=>go("history")}>View history <ChevronRight/></button></div>
...
function AdminDashboard({go,attendance,session,students,gatePasses=[],certificateRequests=[],hostelComplaints=[],notices=[]}){
  const pendingGatePasses = gatePasses.filter(g=>g.status==="Pending").length;
  const pendingCerts = certificateRequests.filter(c=>c.status==="Submitted"||c.status==="Pending").length;
  const openComplaints = hostelComplaints.filter(c=>c.status==="Open"||c.status==="In Progress").length;
  ...
  return <>
    <PageTitle eyebrow="CAMPUSFLOW · ADMIN CONSOLE" title="Campus Operations Center" desc="Monitor student requests, AI-routed complaints, attendance, and facility workflows in real time." actions={<button className="primary" onClick={()=>go("create")}><Plus/> Create Session</button>}/>
    
    <div className="stats-grid">
      <Stat icon={Clock3} label="Pending Requests" value={totalPending} sub={`${pendingGatePasses} Gate · ${pendingCerts} Certs · ${openComplaints} Issues`}/>
      <Stat icon={AlertTriangle} label="Complaints & Ageing" value={openComplaints} sub="2 under 24h · 1 ageing >2d"/>
      <Stat icon={Activity} label="Resolution Time" value="3.2 hrs" sub="↑ 18% faster resolution"/>
      <Stat icon={Zap} label="Recurring Issues" value={recurringIssues.length} sub={recurringIssues.length ? "AI flag: Cluster detected!" : "No clusters flagged"}/>
    </div>
      ...
      <div className="recurring-issue-alert">
        <div className="recurring-alert-head">
          ...
          <div>
            ...
            <p>Multiple complaints received for the same category and location within 72 hours.</p>
          </div>
        </div>
        <div className="recurring-grid">
          {recurringIssues.map(g => (
            <div key={g.key} className="recurring-chip">
              ...
              <button className="ghost mini-btn" onClick={()=>go("adminhostel")}>Investigate</button>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="panel">
      ...
      <div className="admin-action-grid">
        ...
        <button onClick={()=>go("monitor")}>
          <Activity size={20}/>
          <b>Live Presence Monitoring</b>
          <span>Classroom snapshots</span>
          <ChevronRight/>
        </button>
      </div>
    </section>
    ...
    <div className="dashboard-grid">
      <section className="panel">
        <div className="panel-head">
          <div>
            <h3>Staff Workload & Assignment</h3>
        ...
        <div className="staff-workload-list">
          {[
            { name: "Rajesh Kumar", dept: "Electrical Dept", active: 2, resolved: 14, efficiency: "96%" },
            { name: "Suresh Pal", dept: "Plumbing Dept", active: 1, resolved: 11, efficiency: "92%" },
            { name: "Anita Devi", dept: "Housekeeping", active: 0, resolved: 19, efficiency: "98%" },
            { name: "Ramesh Verma", dept: "Carpentry & Furniture", active: 1, resolved: 8, efficiency: "90%" }
          ].map(st => (
            <div key={st.name} className="staff-workload-row">
              <div className="staff-avatar">{initials(st.name)}</div>
              <div className="staff-info">
                <b>{st.name}</b>
                <span>{st.dept} · {st.efficiency} satisfaction</span>
              </div>
      ...
      <section className="panel admin-chart">
        <div className="panel-head">
          <div><h3>Campus Weekly Activity</h3><p>Attendance & service requests</p></div>
          <BarChart3/>
        </div>
        <div className="bars">
          {[74,81,78,92,88,94,87].map((v,i)=><div key={i}><span style={{height:v+"%"}}/><small>{["M","T","W","T","F","S","S"][i]}</small></div>)}
        </div>
      </section>
    </div>
...
function Scanner({attendance,setAttendance,session,studentName}){
 ...
 const [scanner,setScanner]=useState(null); const [camActive,setCamActive]=useState(false); const [camError,setCamError]=useState("");
 const [permissionGranted,setPermissionGranted]=useState(false);

 const markAttendance=(sid)=>{
   if(!sid)return;
   const cleanSid=sid.split("?")[0].split("#")[0].trim();
   if(!cleanSid.startsWith("CAMPUS-ATT-")){setState("error");setMessage("Invalid QR Code. Please scan the official classroom attendance QR.");stopScanner();return;}
   if(attendance.some(a=>a.session===cleanSid)){setState("duplicate");setMessage("Attendance already marked for this session.");stopScanner();return;}
   const next={date:new Date().toISOString().slice(0,10),subject:session?.subject||"Data Structures",teacher:"Dr. Sharma",time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),room:session?.room||"204",status:"Present",session:cleanSid};
   setAttendance([next,...attendance]);setState("success");setMessage("Attendance Marked!");stopScanner();
 };

 const stopScanner=()=>{
   if(scanner){try{scanner.stop().catch(()=>{});scanner.clear().catch(()=>{})}catch{}setScanner(null)}
   setCamActive(false);
 };

 const startCamera=async()=>{
   setCamError("");setState("idle");setMessage("");
   // Use html5-qrcode with the rear/environment facing camera
   ...
   try{
     ...
     await qrScanner.start(
       ...
       (decodedText)=>{
         // When QR is decoded successfully, parse the session ID from the URL
       ...
       ()=>{} // QR scan error callback - ignore individual frame errors
     );
     setCamActive(true);setPermissionGranted(true);
   }catch(err){
     console.error("QR scanner error:",err);
     setCamActive(false);setPermissionGranted(false);
     setCamError(err?.name==="NotAllowedError"||err?.name==="PermissionDeniedError"?"Camera permission denied. Please allow camera access in your browser settings and try again.":"Could not start the camera. Make sure you are using HTTPS and allow camera access.");
   }
 };

 useEffect(()=>()=>stopScanner(),[scanner]);
 ...
 <div className="scanner-layout"><section className="panel scanner-panel"><div className="scanner-window">{camActive?<div id="qr-scanner-region" className="qr-scanner-region"/>:<><div className="scanner-corners"/><QrCode size={72}/><div className="scan-line"/><span>CAMERA / QR SCANNER</span></>}</div><div className="scanner-status">{camActive?<><Camera/><div><b>Camera active</b><span>Position the official classroom QR inside the frame.</span></div></>:<><Camera/><div><b>{camError?"Camera unavailable":"Camera ready"}</b><span>{camError||"Press Start Camera to begin scanning."}</span></div></>}</div>
 {!camActive&&<button className="primary big" onClick={startCamera}><Camera/> Start Camera</button>}
 {camActive&&<button className="ghost" onClick={stopScanner}><X/> Stop Camera</button>}
 {camError&&<div className="result error"><div><AlertTriangle/></div><div><b>Camera error</b><span>{camError}</span></div></div>}
 <div className="demo-input"><label>Manual session ID entry</label><div><input value={code} onChange={e=>setCode(e.target.value)} placeholder="CAMPUS-ATT-DS-58342"/><button className="primary" onClick={scan}>Mark Present</button></div></div>
 <div className="demo-note">Camera uses your device's rear camera for QR scanning. Works on Android Chrome and iPhone Safari when served over HTTPS.</div></section>
...
function Generator({session,setSession}){
 ...
 const generate=async()=>{const id=`CAMPUS-ATT-${form.subject.split(" ")[0].slice(0,3).toUpperCase()}-${Math.floor(10000+Math.random()*89999)}`;const s={...form,id,created:new Date().toISOString(),present:0};setSession(s);
 // Generate QR containing the full public HTTPS URL so students can open the attendance page
 const qrLink=generateAttendanceQRLink(id, s.section, s.room);
 ...
 <section className="panel qr-card">{qr?<><div className="qr-wrap"><img src={qr}/></div><div className="eyebrow">ACTIVE SESSION</div><h3>{session.id}</h3><p className="qr-link-note">Students scan this QR to open the Student Attendance page on their phone.</p><div className="qr-meta"><span>Subject <b>{session.subject}</b></span><span>Section <b>{session.section}</b></span><span>Room <b>{session.room}</b></span><span>Expiry <b>{session.expiry} min</b></span></div><div className="qr-buttons"><button className="primary" onClick={download}><Download/> Download</button><button className="ghost" onClick={()=>window.print()}><Printer/> Print</button><button className="danger" onClick={()=>{setSession(null);setQr("")}}>Close Session</button></div></>:<div className="empty-qr"><QrCode size={72}/><h3>Your QR appears here</h3><p>Fill the session details and generate a scannable QR.</p></div>}</section></div></>
}
...
function SettingsPage({theme,setTheme,logout,profile,role}){return <><PageTitle eyebrow="PREFERENCES" title="Settings" desc="Control your Campus Connect prototype experience."/><section className="panel settings"><div className="setting-row"><div><b>Profile</b><span>{profile.name} · {role==="admin"?"System Administrator":role==="staff"?"Staff Member":"Face verified student"}</span></div><UserRound/></div><div className="setting-row"><div><b>Theme preference</b><span>Switch between light and dark interface.</span></div><button className="toggle" onClick={()=>setTheme(theme==="dark"?"light":"dark")}>{theme==="dark"?<Moon/>:<Sun/>}<span>{theme==="dark"?"Dark":"Light"}</span></button></div><div className="setting-row"><div><b>Camera permission</b><span>Camera is requested only during demo identity verification and QR scanning.</span></div><Camera/></div><div className="setting-row"><div><b>Prototype storage</b><span>Attendance, issues and settings are stored in this browser using localStorage.</span></div><ShieldCheck/></div><button className="danger big" onClick={logout}><LogOut/> Logout</button></section></>}
    ...
    <div className={`workflow-stepper stepper-${color}`}>
      <div className="stepper-track">
        {steps.map((step, idx) => {
          ...
          return (
            <div key={idx} className={`stepper-node ${isDone ? "done" : isCurrent ? "active" : "pending"}`}>
  ...
  const workflows = [
    {
      ...
      steps: [
        { num: 1, actor: "Student", title: "Submits complaint", desc: "Student submits complaint (e.g. 'fan not working')" },
        { num: 2, actor: "AI Engine", title: "AI classifies & routes", desc: "Detects category, location, priority & department" },
        { num: 3, actor: "Admin", title: "Admin reviews & assigns", desc: "Warden/Admin reviews and dispatches staff" },
        { num: 4, actor: "Staff", title: "Staff resolves issue", desc: "Technician resolves problem and updates status" },
        { num: 5, actor: "Student", title: "Student gets notification", desc: "Student verifies fix & confirms closure" }
      ]
    },
    {
      id: "certificate",
      ...
      steps: [
        { num: 1, actor: "Student", title: "Applies for certificate", desc: "Selects type (e.g. Bonafide) & purpose" },
        { num: 2, actor: "System", title: "Form submitted", desc: "Request ID generated with instant tracking" },
        { num: 3, actor: "Admin", title: "Admin verifies", desc: "Admin reviews eligibility and approves / rejects" },
        { num: 4, actor: "System", title: "Certificate generated", desc: "Digital PDF created with official seal & QR" },
        { num: 5, actor: "Student", title: "Student notified", desc: "Ready for download or office collection" }
      ]
    },
    {
      id: "gatepass",
      ...
      statusBadge: "Status: Completed",
      steps: [
        { num: 1, actor: "Student", title: "Applies for gate pass", desc: "Inputs date, out time, return time & reason" },
        { num: 2, actor: "Warden", title: "Warden / Admin reviews", desc: "Approves or rejects with comments" },
        { num: 3, actor: "Security", title: "Security verifies at gate", desc: "Digital QR pass scanned at campus gate" },
        { num: 4, actor: "Gate Log", title: "Gate entry recorded", desc: "Exit & return timestamps recorded in log" },
        { num: 5, actor: "Student", title: "Student notified", desc: "Entry confirmed, pass marked completed" }
      ]
    },
    {
      id: "notice",
      ...
      steps: [
        { num: 1, actor: "Admin", title: "Admin creates notice", desc: "Drafts campus announcement & alerts" },
        { num: 2, actor: "Admin", title: "Selects target audience", desc: "Filters by Branch / Year / Batch / Hostel" },
        { num: 3, actor: "System", title: "Message published", desc: "Stored in database & indexed for delivery" },
        { num: 4, actor: "Engine", title: "System sends notifications", desc: "In-app alerts, SMS & email dispatches" },
        { num: 5, actor: "Student", title: "Students receive & read", desc: "Delivered with read receipts & action tracking" }
      ]
    },
    {
      id: "ai_routing",
      ...
      steps: [
        { num: 1, actor: "Student", title: "Writes complaint", desc: "e.g. 'Hostel bathroom me water leak hai'" },
        { num: 2, actor: "NLP AI", title: "Detects & classifies", desc: "Category, Location, Priority, Department" },
        { num: 3, actor: "Router", title: "Auto-routes to department", desc: "Assigned directly to specialized department" },
        { num: 4, actor: "Pattern AI", title: "Group similar complaints", desc: "Groups by Hostel + Location + Category" },
        { num: 5, actor: "Alert", title: "Mark as Recurring Issue", desc: "Flags critical infrastructure hotspots in Admin Dashboard" }
      ]
    }
  ];
    ...
    <section className="panel workflows-panel">
      <div className="panel-head">
        <div>
          ...
          <p>Seamless 5-step lifecycles powering student services, administrative oversight, and staff resolutions.</p>
        </div>
      ...
      <div className={`workflow-detail-box wf-${workflows[activeTab].color}`}>
        <div className="wf-detail-head">
          ...
          <span className="wf-status-pill">{workflows[activeTab].statusBadge}</span>
        </div>
        <div className="wf-cards-grid">
          {workflows[activeTab].steps.map((st) => (
            <div key={st.num} className="wf-card">
              ...
              <b>{st.title}</b>
              <p>{st.desc}</p>
              {st.num < 5 && <div className="wf-connector"><ArrowRight size={14} /></div>}
            </div>
          ))}
        </div>
      </div>
    ...
    <footer className="accessibility-bar">
      ...
      <div className="access-items">
        ...
        <div className="access-item">
          <Building2 size={14} />
          <span>Language:
            <select value={lang} onChange={(e) => setLang(e.target.value)} className="lang-mini-select">
      ...
      <div className="access-tagline">
        Simple Steps ➔ Complete Workflows ➔ Better Campus Life
      </div>
    </footer>
  );
}
...
function NoticesPage({ notices }) {
  ...
  const filtered = notices.filter(n => {
    ...
    const matchQuery = `${n.title} ${n.body} ${n.target} ${n.author}`.toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <>
      ...
      <div className="notices-grid">
        {filtered.map(notice => (
          <div key={notice.id} className="notice-card" onClick={() => setSelectedNotice(notice)}>
            <div className="notice-card-top">
              <span className={`badge-cat cat-${notice.category.toLowerCase()}`}>{notice.category}</span>
              <span className="target-pill"><Users size={12} /> {notice.target}</span>
            </div>
            <h4>{notice.title}</h4>
            <p>{notice.body}</p>
            <div className="notice-card-footer">
              <span><b>{notice.author}</b> · {notice.date}</span>
              <button type="button" className="ghost mini-btn">Read Full <ChevronRight size={14} /></button>
            </div>
          </div>
        ))}
        {!filtered.length && <div className="demo-note">No notices found matching your criteria.</div>}
      </div>
        ...
        <div className="modal-backdrop" onClick={() => setSelectedNotice(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div className="eyebrow">{selectedNotice.category.toUpperCase()} ANNOUNCEMENT</div>
              <h3>{selectedNotice.title}</h3>
              <button type="button" className="icon-btn" onClick={() => setSelectedNotice(null)}><X size={18} /></button>
            </div>
            <div className="modal-body">
              <div className="notice-meta-bar">
                <span><b>Author:</b> {selectedNotice.author}</span>
                <span><b>Date:</b> {selectedNotice.date}</span>
                <span><b>Target:</b> {selectedNotice.target}</span>
              </div>
              <p className="notice-body-text">{selectedNotice.body}</p>
              <div className="tracking-receipt">
                <CheckCircle2 size={15} /> Read confirmation logged. Action tracked in student audit log.
              </div>
            </div>
            <div className="modal-footer">
              <button type="button" className="primary" onClick={() => setSelectedNotice(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
...
function GatePassPage({ profile, gatePasses, setGatePasses, setNotifications }) {
  ...
  const apply = (e) => {
    ...
    setNotifications(prev => [
      { id: Date.now(), title: "Gate Pass Applied", text: `Your gate pass application ${newPassId} has been sent to the Warden for review.`, read: false },
      ...prev
    ]);
  };

  const simulateGateExit = (passId) => {
    const updated = gatePasses.map(p => {
      if (p.id === passId) {
        return { ...p, status: "Completed", gateVerified: true, exitTime: new Date().toLocaleTimeString() };
      }
      return p;
    });
    setGatePasses(updated);
    ...
    setNotifications(prev => [
      { id: Date.now(), title: "Gate Entry Recorded", text: `Gate exit & re-entry confirmed for Pass ${passId}. Logged in security register.`, read: false },
      ...prev
    ]);
  };
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · LEAVE & MOBILITY"
        title="Leave / Gate Pass Portal"
        desc="Apply for off-campus leave, monitor warden approvals, and show your verified digital gate pass."
      />

      <WorkflowStepper
        steps={["1. Student applies", "2. Warden reviews", "3. Security verifies", "4. Gate entry recorded", "5. Completed"]}
        currentStep={2}
        color="orange"
      />
      ...
      <div className="two-col-layout">
        <section className="panel">
          <div className="panel-head">
            <div>
              ...
              <p>Direct submission to Hostel Warden & Security</p>
            </div>
            <DoorOpen size={20} />
          </div>
          <form onSubmit={apply} className="cf-form">
            <div className="form-grid">
              ...
              <label>
                Out Time
                <input type="text" value={time} onChange={e => setTime(e.target.value)} placeholder="02:00 PM" required />
              </label>
            </div>
            <div className="form-grid">
              <label>
              ...
              <label>
                ...
                <input type="text" value={returnTime} onChange={e => setReturnTime(e.target.value)} placeholder="07:00 PM" required />
              </label>
            </div>
            <label>
              Reason for Leave
              <input value={reason} onChange={e => setReason(e.target.value)} placeholder="e.g. Medical checkup, family visit, project supplies" required />
            </label>
            <label>
              Parent / Emergency Contact Number
              <input value={contact} onChange={e => setContact(e.target.value)} placeholder="10-digit mobile number" required />
            </label>
            <button type="submit" className="primary big"><DoorOpen size={16} /> Submit Gate Pass Application</button>
          </form>
          {successId && (
            <div className="success-box">
              <CheckCircle2 size={16} /> Gate Pass Application #{successId} submitted! Warden notified.
            </div>
          )}
        </section>
        ...
        <section className="panel">
          <div className="panel-head">
            ...
            <span className="attendance-pill">{gatePasses.length} records</span>
          </div>
            ...
            {gatePasses.map(pass => (
              <div key={pass.id} className="pass-card">
                <div className="pass-card-top">
                  <b>#{pass.id}</b>
                  <span className={`status-badge status-${pass.status.toLowerCase()}`}>{pass.status}</span>
                </div>
                <p className="pass-reason">{pass.reason}</p>
                <div className="pass-timing">
                  <span>Out: <b>{pass.date} {pass.time}</b></span>
                  <span>Return: <b>{pass.returnDate} {pass.returnTime}</b></span>
                </div>
                {pass.approvedBy && <small className="approved-by">Approved by: {pass.approvedBy}</small>}
                <div className="pass-actions">
                  {pass.status === "Approved" && (
                    <button type="button" className="primary mini-btn" onClick={() => setSelectedPass(pass)}>
                      <QrCode size={14} /> Show Digital Pass
                    </button>
                  )}
                  {pass.status === "Approved" && (
                    <button type="button" className="ghost mini-btn" onClick={() => simulateGateExit(pass.id)}>
                      <ShieldCheck size={14} /> Simulate Gate Exit
                    </button>
                  )}
                </div>
              </div>
      ...
      {selectedPass && (
        <div className="modal-backdrop" onClick={() => setSelectedPass(null)}>
          <div className="modal-box digital-pass-modal" onClick={e => e.stopPropagation()}>
            <div className="digital-pass-card">
              ...
              <p className="pass-student-id">{selectedPass.studentId} · Hostel Block</p>
              <div className="pass-qr-box">
                <QrCode size={120} />
                <span>SCAN AT MAIN GATE TERMINAL</span>
              </div>
              <div className="pass-details-grid">
                <div><span>Pass ID:</span><b>#{selectedPass.id}</b></div>
                <div><span>Status:</span><b className="text-green">{selectedPass.status}</b></div>
                <div><span>Out Time:</span><b>{selectedPass.date} {selectedPass.time}</b></div>
                <div><span>Return By:</span><b>{selectedPass.returnDate} {selectedPass.returnTime}</b></div>
                <div className="col-span-2"><span>Reason:</span><b>{selectedPass.reason}</b></div>
                <div className="col-span-2"><span>Warden Clearance:</span><b>{selectedPass.approvedBy || "Verified"}</b></div>
              </div>
              <button type="button" className="primary big" onClick={() => setSelectedPass(null)}>Close Pass</button>
            </div>
          </div>
        </div>
      )}
...
function CertificateRequestPage({ profile, certificateRequests, setCertificateRequests }) {
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · ACADEMIC SERVICES"
      ...
      <WorkflowStepper
        steps={["1. Student applies", "2. Form submitted (ID)", "3. Admin verifies", "4. PDF generated", "5. Ready to collect"]}
        currentStep={2}
        color="blue"
      />
      ...
      <div className="two-col-layout">
        <section className="panel">
          <div className="panel-head">
            <div>
              ...
              <p>Generated with university seal and digital verification hash</p>
            </div>
            <Award size={20} />
          </div>
          <form onSubmit={submitRequest} className="cf-form">
            ...
            <label>
              Purpose / Reason
              <textarea value={purpose} onChange={e => setPurpose(e.target.value)} placeholder="e.g. Bank loan application, Passport renewal, Internship verification" required />
            </label>
            <button type="submit" className="primary big"><Award size={16} /> Submit Certificate Request</button>
          </form>
          {successId && (
            <div className="success-box">
              <CheckCircle2 size={16} /> Request submitted! Tracking ID: <b>#{successId}</b>. Admin notified for verification.
            </div>
          )}
        </section>
        ...
        <section className="panel">
          <div className="panel-head">
            ...
            <span className="attendance-pill">{certificateRequests.length} requests</span>
          </div>
          <div className="passes-list">
            {certificateRequests.map(cert => (
              <div key={cert.id} className="pass-card">
                ...
                <div className="pass-timing">
                  <span>Applied: <b>{cert.requestDate}</b></span>
                  {cert.completedDate && <span>Issued: <b>{cert.completedDate}</b></span>}
                </div>
                {cert.status === "Generated" && (
                  <button type="button" className="primary mini-btn" onClick={() => setPreviewCert(cert)}>
      ...
      {previewCert && (
        <div className="modal-backdrop" onClick={() => setPreviewCert(null)}>
          <div className="modal-box cert-preview-modal" onClick={e => e.stopPropagation()}>
            <div className="official-certificate-doc">
              <div className="cert-border">
                <div className="cert-header">
                  <div className="cert-emblem"><Building2 size={32} /></div>
                  <h2>CAMPUSFLOW UNIVERSITY OF TECHNOLOGY</h2>
                  <p>Accredited 'A++' Grade · Office of Academic Affairs & Registrar</p>
                  <div className="cert-title-badge">{previewCert.type.toUpperCase()}</div>
                </div>
                <div className="cert-content-text">
                  <p>This is to certify that <b>{previewCert.studentName}</b>, bearing Student ID <b>{previewCert.studentId}</b>, is a bonafide student of this Institute pursuing Bachelor of Technology (B.Tech) in Computer Science & Engineering.</p>
                  <p>This certificate is officially issued on this day upon the student's request for the purpose of: <i>{previewCert.purpose}</i>.</p>
                  <p>His / Her conduct and character during the period of study have been observed to be GOOD.</p>
                </div>
                <div className="cert-signatures">
                  <div className="cert-qr">
                    <QrCode size={64} />
                    <small>Verify: campusflow.edu/verify/{previewCert.id}</small>
                  </div>
            ...
            <div className="modal-footer">
              <button type="button" className="ghost" onClick={() => window.print()}><Printer size={15} /> Print Certificate</button>
              <button type="button" className="primary" onClick={() => setPreviewCert(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
...
function HostelComplaintPage({ profile, complaints, setComplaints, setNotifications }) {
  ...
  const submitComplaint = (e) => {
    ...
    setDesc("");
    setNotifications(prev => [
      { id: Date.now(), title: "Complaint Routed via AI", text: `Your issue has been automatically classified as ${ai.category} and routed to ${ai.department}.`, read: false },
      ...prev
    ]);
  };
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · SMART HOSTEL MAINTENANCE"
      ...
      <WorkflowStepper
        steps={["1. Student submits", "2. AI classifies & routes", "3. Admin assigns staff", "4. Staff resolves", "5. Closed"]}
        currentStep={1}
        color="red"
      />
      ...
      <div className="two-col-layout">
        <section className="panel">
          ...
          <form onSubmit={submitComplaint} className="cf-form">
            <label>
              Hostel Room No.
              <input value={room} onChange={e => setRoom(e.target.value)} placeholder="e.g. B-204" required />
            </label>
            <label>
              Describe the Issue
              <textarea
            ...
            {liveAI && (
              <div className="live-ai-box">
                ...
                <div className="ai-pills">
                  <span className="ai-pill">Category: <b>{liveAI.category}</b></span>
                  <span className="ai-pill">Location: <b>{liveAI.location}</b></span>
                  <span className={`ai-pill priority-${liveAI.priority.toLowerCase()}`}>Priority: <b>{liveAI.priority}</b></span>
                  <span className="ai-pill dept">Routed to: <b>{liveAI.department}</b></span>
                </div>
              </div>
            )}

            <button type="submit" className="primary big"><AlertTriangle size={16} /> Submit & Route via AI</button>
          </form>
          {submittedId && (
            <div className="success-box">
              <CheckCircle2 size={16} /> Complaint #{submittedId} logged! AI routed it directly to the designated department.
            </div>
          )}
        </section>
              ...
              <div key={comp.id} className="pass-card">
                <div className="pass-card-top">
                  ...
                  <div className="badge-cluster">
                    <span className={`badge-cat cat-${comp.aiCategory?.toLowerCase()}`}>{comp.aiCategory}</span>
                    <span className={`status-badge status-${comp.status.toLowerCase().replace(" ", "")}`}>{comp.status}</span>
                  </div>
                </div>
                <p className="pass-reason">{comp.description}</p>
                <div className="complaint-ai-meta">
                  <span>📍 {comp.aiLocation}</span>
                  <span>⚡ Priority: <b>{comp.aiPriority}</b></span>
                  <span>🏢 Dept: <b>{comp.aiDepartment}</b></span>
                </div>
                {comp.assignedTo && <div className="assigned-note">Technician: <b>{comp.assignedTo}</b></div>}
                {comp.remarks && <div className="remarks-note">Remarks: <i>"{comp.remarks}"</i></div>}
                {comp.status === "Resolved" && (
                  <button type="button" className="primary mini-btn" onClick={() => confirmClosure(comp.id)}>
...
function MessMenuPage() {
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · DINING & LIVING"
      ...
      <div className="panel mess-info-panel">
        ...
        <div className="mess-rules-grid">
          <div><b>Dietary Hygiene:</b> FSSAI certified central campus kitchen. RO drinking water plants in all dining halls.</div>
          <div><b>Sunday Special:</b> Extended grand lunch buffet with chef's special dessert and regional delicacies.</div>
          <div><b>Feedback Desk:</b> Leave immediate food ratings in the CampusFlow Feedback portal.</div>
        </div>
      </div>
    </>
  );
...
// ============================================================================
// STAFF PORTAL PAGES (Resolve & Update)
// ============================================================================

function StaffDashboard({ go, complaints, profile, setComplaints }) {
  const staffName = profile.name || "Rajesh Kumar";
  const myComplaints = complaints.filter(c => !c.assignedTo || c.assignedTo === staffName || c.assignedTo.includes(staffName.split(" ")[0]));
  const openCount = myComplaints.filter(c => c.status === "Open" || c.status === "In Progress").length;
  ...
  return (
    <>
      <PageTitle
        ...
        actions={<button className="primary" onClick={() => go("staffcomplaints")}><Wrench size={16} /> View Assigned Work</button>}
      />

      <div className="stats-grid">
        <Stat icon={Wrench} label="Assigned Complaints" value={myComplaints.length} sub="Total allocated to you" />
        <Stat icon={Clock3} label="Pending Action" value={openCount} sub="Needs your attention" />
        <Stat icon={CheckCircle2} label="Resolved Tickets" value={resolvedCount} sub="Completed this cycle" />
        <Stat icon={Activity} label="Avg Resolution Time" value="2.4 hrs" sub="Exceeding department SLA" />
      </div>
      ...
      <section className="panel">
        <div className="panel-head">
          ...
          <button className="ghost" onClick={() => go("staffcomplaints")}>Open full manager <ChevronRight size={14} /></button>
        </div>
        <div className="staff-queue-list">
          {myComplaints.slice(0, 4).map(c => (
            <div key={c.id} className="staff-queue-card">
              <div className="queue-card-top">
                <b>#{c.id} · {c.aiLocation}</b>
                <span className={`status-badge status-${c.status.toLowerCase().replace(" ", "")}`}>{c.status}</span>
              </div>
              <p>{c.description}</p>
              <div className="queue-card-footer">
                <span>Category: <b>{c.aiCategory}</b> · Priority: <b className={`text-${c.aiPriority.toLowerCase()}`}>{c.aiPriority}</b></span>
                <button type="button" className="primary mini-btn" onClick={() => go("staffcomplaints")}>
                  Update Status <ChevronRight size={13} />
                </button>
              </div>
            </div>
          ))}
          {!myComplaints.length && <div className="demo-note">No assigned complaints in your queue. Great job!</div>}
        </div>
      </section>
...
function StaffComplaintsPage({ complaints, setComplaints, profile }) {
    ...
    setComplaints(complaints.map(c => {
      if (c.id === id) {
        return {
          ...
          remarks: remark || c.remarks || `Status set to ${newStatus} by ${staffName}`
        };
      }
      return c;
    }));
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · TECHNICIAN WORKBENCH"
          ...
          <div key={c.id} className="staff-ticket-card">
            <div className="ticket-header">
              <div>
                ...
                <h4>{c.aiCategory} issue in {c.aiLocation}</h4>
              </div>
              <span className={`status-badge status-${c.status.toLowerCase().replace(" ", "")}`}>{c.status}</span>
            </div>
            ...
            <div className="ticket-details-box">
              <div><span>Student:</span> <b>{c.studentName} ({c.studentId})</b></div>
              <div><span>Department:</span> <b>{c.aiDepartment}</b></div>
              <div><span>Priority:</span> <b className={`priority-tag ${c.aiPriority.toLowerCase()}`}>{c.aiPriority}</b></div>
              <div><span>Logged:</span> <b>{c.createdAt}</b></div>
            </div>
...
function AdminNoticesPage({ notices, setNotices }) {
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · BROADCAST ENGINE"
      ...
      <WorkflowStepper
        steps={["1. Admin creates", "2. Target selected", "3. Published", "4. Sent via engine", "5. Read tracking"]}
        currentStep={2}
        color="purple"
      />
      ...
      <div className="two-col-layout">
        <section className="panel">
          ...
          <form onSubmit={publish} className="cf-form">
            <label>
              Announcement Title
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Hostel Mess Timings Revised" required />
            </label>
            <div className="form-grid">
              <label>
                Category
            ...
            <label>
              Notice Body
              <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Detailed message..." rows={4} required />
            </label>
            <button type="submit" className="primary big"><Send size={16} /> Publish & Notify Target Audience</button>
          </form>
          {msg && <div className="success-box"><CheckCircle2 size={16} /> {msg}</div>}
        </section>
          ...
          <div className="passes-list">
            {notices.map(n => (
              <div key={n.id} className="pass-card">
                <div className="pass-card-top">
                  <b>{n.title}</b>
                  <button type="button" className="danger mini-btn" onClick={() => deleteNotice(n.id)}><Trash2 size={13} /></button>
                </div>
                <p className="pass-reason">{n.body}</p>
                <div className="notice-reach-bar">
                  <span>Target: <b>{n.target}</b></span>
                  <span>Category: <b>{n.category}</b></span>
                  <span>Delivered: <b>1,240</b></span>
                  <span className="text-green">Read: <b>89%</b></span>
                </div>
              </div>
            ))}
          </div>
...
function AdminGatePassPage({ gatePasses, setGatePasses, setNotifications }) {
  const [filter, setFilter] = useState("All");

  const filtered = gatePasses.filter(p => {
  ...
  const updateStatus = (id, newStatus) => {
    setGatePasses(gatePasses.map(p => {
      if (p.id === id) {
        return { ...p, status: newStatus, approvedBy: "Warden Office / Admin" };
      }
      return p;
    }));
    setNotifications(prev => [
      { id: Date.now(), title: `Gate Pass ${newStatus}`, text: `Gate pass #${id} has been ${newStatus.toLowerCase()} by the administration.`, read: false },
      ...prev
    ]);
  };
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · WARDEN & SECURITY CONSOLE"
        title="Gate Pass Management"
        desc="Review student leave requests, issue warden approvals, and monitor campus gate entry logs."
      />
          ...
          <table>
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id}>
                  <td><b>#{p.id}</b></td>
                  <td><b>{p.studentName}</b><br /><small>{p.studentId}</small></td>
                  <td>{p.date} {p.time}</td>
                  <td>{p.returnDate} {p.returnTime}</td>
                  <td>{p.reason}</td>
                  <td>{p.emergencyContact || "9876543210"}</td>
                  <td><span className={`status-badge status-${p.status.toLowerCase()}`}>{p.status}</span></td>
                  <td>
                    {p.status === "Pending" && (
                      <div className="action-button-group">
                        <button type="button" className="primary mini-btn" onClick={() => updateStatus(p.id, "Approved")}>
                          <CheckCircle2 size={13} /> Approve
                        </button>
                        <button type="button" className="danger mini-btn" onClick={() => updateStatus(p.id, "Rejected")}>
                          <X size={13} /> Reject
                        </button>
                      </div>
                    )}
                    {p.status === "Approved" && (
                      <button type="button" className="ghost mini-btn" onClick={() => updateStatus(p.id, "Completed")}>
                        <DoorOpen size={13} /> Log Gate Exit
                      </button>
                    )}
                  </td>
                </tr>
...
function AdminCertificatePage({ certificateRequests, setCertificateRequests }) {
  ...
  const verifyAndGenerate = (id) => {
    setCertificateRequests(certificateRequests.map(c => {
      if (c.id === id) {
        return {
          ...c,
          status: "Generated",
  ...
  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · REGISTRAR DESK"
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id}>
                  ...
                  <td>
                    {c.status === "Submitted" ? (
                      <button type="button" className="primary mini-btn" onClick={() => verifyAndGenerate(c.id)}>
                        <Award size={13} /> Approve & Generate PDF
                      </button>
                    ) : (
                      <span className="text-green"><CheckCircle2 size={13} /> Ready for Download</span>
                    )}
                  </td>
                </tr>
              ))}
...
function AdminHostelComplaintPage({ complaints, setComplaints }) {
  ...
  const staffMembers = ["Rajesh Kumar (Electrical)", "Suresh Pal (Plumbing)", "Anita Devi (Housekeeping)", "Ramesh Verma (Carpentry)"];

  const assignStaff = (id, staff) => {
    setComplaints(complaints.map(c => c.id === id ? { ...c, assignedTo: staff, status: "In Progress" } : c));
  };

  return (
    <>
      <PageTitle
        eyebrow="CAMPUSFLOW · FACILITY OPERATIONS"
        ...
        <div className="recurring-issue-alert">
          <div className="recurring-alert-head">
            ...
            <div>
              ...
              <p>The pattern engine detected repetitive issues in the same hostel location. Immediate inspection recommended.</p>
            </div>
          </div>
          <div className="recurring-grid">
            {recurring.map(r => (
          ...
          <table>
            ...
            <tbody>
              {complaints.map(c => (
                <tr key={c.id}>
                  <td><b>#{c.id}</b></td>
                  <td><b>{c.studentName}</b><br /><small>{c.aiLocation}</small></td>
                  <td>{c.description}</td>
                  <td><span className={`badge-cat cat-${c.aiCategory?.toLowerCase()}`}>{c.aiCategory}</span></td>
                  <td><span className={`priority-tag ${c.aiPriority?.toLowerCase()}`}>{c.aiPriority}</span></td>
                  <td><b>{c.aiDepartment}</b></td>
                  <td>
                    <select
                      value={c.assignedTo || ""}
...
function StudentAttendance(){
  ...
  const markAttendance=(sid)=>{
    ...
    const cleanSid=sid.split("?")[0].split("#")[0].trim();
    if(!cleanSid.startsWith("CAMPUS-ATT-")){setState("error");setMessage("Invalid QR Code. Please scan the official classroom attendance QR.");stopScanner();return;}
    // Check if already marked in this browser
    const stored=load("cc_student_attendance",[]);
    if(stored.some(a=>a.session===cleanSid)){setState("duplicate");setMessage("Attendance already marked for this session.");stopScanner();return;}
    const next={date:new Date().toISOString().slice(0,10),subject:sessionInfo?.class||"Data Structures",teacher:"Dr. Sharma",time:new Date().toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}),room:sessionInfo?.class||"204",status:"Present",session:cleanSid};
    save("cc_student_attendance",[next,...stored]);
    setMarkedSession(cleanSid);setState("success");setMessage("Attendance Marked!");stopScanner();
  };

  const startCamera=async()=>{
    setCamError("");setState("idle");setMessage("");
    const attendanceQrScanner=new Html5Qrcode("student-qr-scanner-region");
    try{
      setScanner(attendanceQrScanner);
      await attendanceQrScanner.start(
        {facingMode:"environment"},
        {fps:10,qrbox:{width:240,height:240}},
        (decodedText)=>{
          let sessionId=decodedText;
  ...
  return <div className="student-attendance-page">
    ...
    <div className="student-attendance-card">
      ...
      <div className="eyebrow">CAMPUS PLUS · ATTENDANCE</div>
      <h1>Mark Attendance</h1>
      <p className="muted">Scan the classroom QR code to record your presence.</p>
      {sessionInfo?.session&&<div className="session-badge"><QrCode size={14}/> Session: {sessionInfo.session}</div>}
      <div className="student-scanner-window">
        {camActive?<div id="student-qr-scanner-region" className="qr-scanner-region"/>:<>
      ...
      <div className="scanner-status">
        ...
        <div>
          ...
          <span>{camActive?"Position the classroom QR inside the frame.":(camError||"Press Start Camera to begin scanning.")}</span>
        </div>
      </div>
      {!camActive&&<button className="primary big" onClick={startCamera}><Camera/> Start Camera</button>}
      {camActive&&<button className="ghost" onClick={stopScanner}><X/> Stop Camera</button>}
      {camError&&<div className="result error"><div><AlertTriangle/></div><div><b>Camera error</b><span>{camError}</span></div></div>}
      {state!=="idle"&&<div className={`result ${state}`}><div>{state==="success"?<CheckCircle2/>:<AlertTriangle/>}</div><div><b>{message}</b>{state==="success"&&<span>Session: {markedSession} · Status: Present</span>}</div></div>}
      <div className="demo-note">Camera uses your device's rear camera. Works on Android Chrome and iPhone Safari when served over HTTPS.</div>
    </div>
  </div>;
}
